...
rank:i.rank?.value.toLocaleLowerCase(),kingdom:i.kingdom?.value,genericName:p,infragenericEpithet:i.infrag?.value,specificEpithet:i.
specific?.value,infraspecificEpithet:i.infrasp?.value,noMissing:!0}}}).filter(i=>i!==void 0))}async function Z(a,t,n){let s=`\
...
continue;let l={colUri:r,acceptedColUri:o,humanReadable:u,authority:h,status:p,latinName:{rank:c.rank?.value.toLocaleLowerCase(),
kingdom:c.kingdom?.value,genericName:e,infragenericEpithet:c.infrag?.value,specificEpithet:c.specific?.value,infraspecificEpithet:c.
infrasp?.value,noMissing:!0}};r===c.acceptedcol?.value?d=l:i.add(l)}if(!d)throw new Error(`Could not get synonyms for Co\
L <${a}> [missing acceptedcol]`);return{accepted:d,synonyms:i}}async function M(a,t,n,s){let g=`
...
WHERE {
    ...
    ${a.infragenericEpithet?`?tn dwc:subGenus|dwc:section|dwc:series "${a.infragenericEpithet}" .`:a.noMissing&&!(t&&a.specificEpithet)?
...
authorized:p?[p]:[],treats:c.tntreats?.value,cites:c.tncites?.value,latinName:{rank:c.rank?.value.toLocaleLowerCase(),kingdom:c.
kingdom?.value,genericName:o,infragenericEpithet:c.infrag?.value,specificEpithet:c.specific?.value,infraspecificEpithet:c.
infrasp?.value,noMissing:!0}})}return new Set(i.values())}async function G(a,t,n){let s=`
PREFIX dc: <http://purl.org/dc/elements/1.1/>
PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>
PREFIX dwcFP: <http://filteredpush.org/ontologies/oa/dwcFP#>
PREFIX cito: <http://purl.org/spar/cito/>
...
authorized:u?[u]:[],treats:i.tntreats?.value,cites:i.tncites?.value,latinName:{rank:i.rank?.value.toLocaleLowerCase(),kingdom:i.
kingdom?.value,genericName:r,infragenericEpithet:i.infrag?.value,specificEpithet:i.specific?.value,infraspecificEpithet:i.
infrasp?.value,noMissing:!0}})}if(d.size!==1)throw new Error(`Got multiple latin names for ${a}`);return d.values().next().
value}async function W(a,t,n){let s=`
PREFIX dc: <http://purl.org/dc/elements/1.1/>
PREFIX dwc: <http://rs.tdwg.org/dwc/terms/>
PREFIX dwcFP: <http://filteredpush.org/ontologies/oa/dwcFP#>
PREFIX cito: <http://purl.org/spar/cito/>
...
kingdom?.value,genericName:r,infragenericEpithet:i.infrag?.value,specificEpithet:i.specific?.value,infraspecificEpithet:i.
infrasp?.value,noMissing:!0}})}if(d.size!==1)throw new Error(`Got multiple latin names for ${a}`);return d.values().next().
value}function U(a,t){let n=a.split(/\s*[,]\s*/),s=t.split(/\s*[,]\s*/),g=n.length>0&&/\d{4}/.test(n.at(-1))?n.pop():null,d=s.
...
p),m=[];console.log(p,l,e);let N,I=[],O,w,S;if(l)for(let f of l.values()){let y=f.colUri;O||(O=f.latinName.kingdom),w||(w=
...
status:f.status,acceptedURI:f.acceptedColUri})}if(e){w||(w=X(e.latinName));for(let f of e.authorized){let y=this.makeTreatmentSet(
...
treatments.dpr.union(_),cite:E.treatments.cite.union(H)}}else I.push({displayName:w,authority:f.authorities,authorities:f.
authorities.split(" / "),taxonConceptURIs:[f.tcUri],treatments:{def:y,aug:R,dpr:_,cite:H}})}}w||(w=p);let P=e?.treats?this.
makeTreatmentSet(e.treats.split("|")):new Set;P.forEach(f=>m.push(f));let C={kingdom:O??e?.latinName.kingdom??"",displayName:w,
...
now(),dt=new D(ot),v=new z(dt,Y,at,st,it),x=class extends HTMLElement{constructor(t,n){super(),this.innerHTML=T[n]??T.unknown;
...
append(" ",e),v.findName(p).then(m=>{e.classList.remove("uri"),m.authority?e.innerText=m.displayName+" "+m.authority:e.innerText=
...
.org/id/","");e.innerText=l,e.href="#"+l,e.title="show name",u.append(" ",e),v.findName(p).then(m=>{e.classList.remove("\
...
xon-name.plazi.org/id/","");e.innerText=l,e.href="#"+l,e.title="show name",u.append(" ",e),v.findName(p).then(m=>{e.classList.
...
e",u.append(" ",e),v.findName(p).then(m=>{e.classList.remove("uri"),m.authority?e.innerText=m.displayName+" "+m.authority:
...
e.href="#"+l,e.title="show name",u.append(" ",e),v.findName(p).then(m=>{e.classList.remove("uri"),m.authority?e.innerText=
...
innerText=l,e.href="#"+l,e.title="show name",u.append(" ",e),v.findName(p).then(m=>{e.classList.remove("uri"),m.authority?
...
catalogueoflife.org/data/taxon/","");m.innerText=N,m.href=`#${N}`,m.title="show name",l.append(m),v.findName(t.col.acceptedURI).
...
createElement("div");if(O.classList.add("indent"),N.append(O),o.col.acceptedURI!==o.col.colURI){let w=document.createElement(
"div");w.innerHTML=T.east+T.col_aug,O.append(w);let S=document.createElement("a");S.classList.add("col","uri");let P=o.col.
acceptedURI.replace("https://www.catalogueoflife.org/data/taxon/","");S.innerText=P,S.href=`#${P}`,S.title="show name",w.
append(" ",S),v.findName(o.col.acceptedURI).then(C=>{S.classList.remove("uri"),C.authority?S.innerText=C.displayName+" "+
...
, according to the Catalogue of Life,
     a synonym of ${a.justification.parent.displayName} which ${t}`}}for await(let a of v){let t=new F(a);Q.append(t)}var pt=performance.
now();k.innerHTML="";k.innerText=`Found ${v.names.length} names with ${v.treatments.size} treatments. This took ${(pt-lt)/
